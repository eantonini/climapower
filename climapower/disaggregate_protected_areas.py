...
for index, country in countries.iterrows():
    # country = countries.loc[countries['Name']=='France'].squeeze()
    
    # Create a folder for the country
    regional_folder_name = 'WDPA_WDOECM_'+dataset_date+'_Public_'+country['ISO Alpha-2']+'_shp/'
    regional_protected_areas_filename = 'WDPA_WDOECM_'+country['ISO Alpha-2']+'_shp.shp'
    ...
    if not os.path.exists(data_path+regional_folder_name+regional_protected_areas_filename):
        ...
        region = geometry.get_geopandas_region(region_shapes, country, dataset_attribute, backup_dataset_attribute, make_plot=False)
